using UnityEngine;
using UnityEngine.AI;   //길찾기(네비메시)를 사용하기 위해서 쓴다.
...
public class FirstEnemyAi : MonoBehaviour
{
    ...
    void Start() 
    { 
        agent = GetComponent<NavMeshAgent>(); //NavMeshAgent가져오기
        anim = GetComponent<Animator>(); 
        currentHealth = maxHealth; //시작할 때 체력을 최대로 설정 //비어있는지 아닌지
       
        //예외처리
        if (agent == null) 
        { Debug.LogWarning("네비메쉬 컴포넌트가 없습니다. EnenmyAI 가 제대로 작동하지 않을 수 있다"); }
